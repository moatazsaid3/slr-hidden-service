...
def modify(packet):
    ...
    extension_header = bytes(keycb.decrypt(bytes(nonce), pkt.all_extension_headers[0].data[30:-43], ''))

    payload = str(keycb.decrypt(bytes(noncep), pkt.data + payload_signature, ''))

    ci = struct.unpack(">I", extension_header[0:4])[0]
    nonce = struct.unpack(">I", extension_header[4:8])[0]
    noncep = struct.unpack(">I", extension_header[8:12])[0]
    payload_signature = str(extension_header[12:28])

    extension_header = bytes(keycs.decrypt(bytes(nonce), extension_header[28:], ''))

    pkt.nxt = ord(extension_header[0])

    payload = str(keycs.decrypt(bytes(noncep), payload + payload_signature, ''))
    pkt.data = payload
